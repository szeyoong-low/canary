locals {
  # The canary.markets zone in Cloudflare
  zone_id = "298ffd9b0153fe3a36795cda6401f8e9"

  # TNamecheap's email-forwarding servers
  mx_records = {
...
# Apex domain. A CNAME at the apex is only legal because Cloudflare flattens it
# into an A record at query time (CNAME flattening).
resource "cloudflare_dns_record" "apex" {
  zone_id = local.zone_id
  name    = "canary.markets"
  type    = "CNAME"
  content = "910dc9cc7bd71536.vercel-dns-017.com"
  proxied = true
  ttl     = 1 # 1 is Cloudflare's sentinel for "Automatic". Required when proxied.
}

resource "cloudflare_dns_record" "www" {
  zone_id = local.zone_id
  name    = "www.canary.markets"
  type    = "CNAME"
  content = "910dc9cc7bd71536.vercel-dns-017.com"
  proxied = true
  ttl     = 1
